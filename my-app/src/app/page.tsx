...
export default function Home() {
  ...
  const categories = useMemo(() => {
    return ['Wszystkie','Sprzedaż','Marketing','Finanse','Bankowość','Inżynieria','Obsługa klienta']
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    return jobs.filter(j => {
      if (category && category !== 'Wszystkie') {
        const hasCategory = (j.category && j.category === category) || (Array.isArray(j.tags) && j.tags.includes(category))
        if (!hasCategory) return false
      }
      if (q) {
        const match = String(j.title ?? '').toLowerCase().includes(q) || String(j.company ?? '').toLowerCase().includes(q) || String(j.description ?? '').toLowerCase().includes(q)
        if (!match) return false
      }
      if (loc) {
        const matchLoc = String(j.location ?? '').toLowerCase().includes(loc)
    ...
    <div className="min-h-screen bg-gray-50">
      <section className="hero-gradient rounded-2xl mx-6 my-8 p-8 md:p-12">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-2">Zdobądź lepszą pracę. W nowym roku.</h1>
            <p className="text-gray-700 mb-6">Szukaj ofert, aplikuj i zarządzaj swoimi aplikacjami w jednym miejscu.</p>
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {categories.map(cat => {
            const isActive = category === cat || (cat === 'Wszystkie' && category === null)
            return (
              <button
                key={cat}
                onClick={() => setCategory(prev => (cat === 'Wszystkie' ? null : (prev === cat ? null : cat)))}
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8">
        ...
        {loading ? (
          ...
          <div className="text-red-600">Błąd podczas pobierania ofert: {fetchError}</div>
        ) : filtered.length === 0 ? (
          <p>Brak ofert pasujących do wyszukiwania.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map((job) => (
              <article key={job.id} className="p-4 bg-white rounded-lg shadow-sm">
                <div>
                  <Link href={`/jobs/${job.id}`} className="no-underline">
                    <h4 className="text-lg font-semibold mb-1 text-black">{job.title}</h4>
                    <div className="text-sm text-black/80 mb-2">{job.company} • {job.location ?? 'Zdalnie'}</div>
                    <p className="text-sm text-black/90 mb-3">{job.description?.slice(0, 160)}{job.description && job.description.length > 160 ? '…' : ''}</p>
                  </Link>
                </div>
